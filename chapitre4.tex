...
\begin{dmd}
\backslash newcommand\{\codereplace{宏名}\}[\codereplace{参量}]\{\codereplace{\LaTeX 代码}\}
\end{dmd}

其中\codereplace{参量}是指\emph{参数}的数量——所谓参数，是编程中的术语。此外，\codereplace{\LaTeX 代码}是指你的指令的定义。使用宏的示例如下，其中定义了测色法中表示空间的符号：
...
注意，这个指令不接受参数，因此没有必要选用参数\codereplace{参量}。可以通过以下方式来改善该指令的使用方法：
...
\begin{dmd}
\backslash newcommand\{\codereplace{宏名}\}[\codereplace{参量}][\codereplace{默认值}]\{\codereplace{\LaTeX 代码}\}
\end{dmd}

其中，\codereplace{参量}依然指参数的数量，参数\verb|#1|带有默认值\codereplace{默认值}，\codereplace{\LaTeX 代码}是指令的内容。以下示例再次实现了前文实现过的效果——显示键盘按键：
...
\begin{dmd}
\backslash newenvironment\{\codereplace{环境名}\}[\codereplace{参量}]\{\codereplace{begin条目}\}\{\codereplace{end条目}\}
\end{dmd}

其中，可以通过\codereplace{环境名}定义环境名，\codereplace{参量}代表参数的数量，\codereplace{起始条目}和\codereplace{结束条目}分别代表环境“开端”和“末尾”所需的处理。在定义环境时使用其他环境（如\LaTeX 的环境）是很实用的技巧\yz{
  正如下文所说，译者也只能查到这句话来自维克多·雨果的说法，但没有查到具体出处，只能照字面意义硬翻。
}：
